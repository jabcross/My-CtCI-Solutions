...
fn pond_sizes(height_map: &str) -> Vec<usize> {
    ...
    let mut height_map: Vec<Vec<i32>> = height_map
        .split_terminator('\n')
        .map(|slice: &str| -> Vec<i32> {
            slice
                .trim()
                .split_ascii_whitespace()
                .map(|text: &str| -> i32 { text.parse::<i32>().unwrap() })
                .collect()
        })
        .collect();
    ...
    fn get_position(height_map: &mut Vec<Vec<i32>>, coords: (i32, i32)) -> Option<&mut i32> {
    ...
    fn flood(height_map: &mut Vec<Vec<i32>>, coords: (i32, i32), id: i32) -> usize {
