...
fn match_pattern<'a>(string: &'a str,
                     pattern: &'a str,
                     pattern_attempts: &mut HashMap<&'a str, &'a str>)->bool {
    ...
    match pattern_attempts.get(&pattern[0..1]) {
        Some(slice) => {
            if string.starts_with(*slice) {
                return match_pattern(&string[slice.len()..],
                                     &pattern[1..],
                                     pattern_attempts);
            }
            else {
        ...
        None => {
            ...
            for length in 1..string.len()+1 {
                ...
                let result = match_pattern(&string[length..],
                                           &pattern[1..],
                                           pattern_attempts);
...
pub fn test() {
    ...
    assert!(match_pattern("dog", "aba", &mut HashMap::new()) == false);
    println!("Ex 16.18 ok!");
}