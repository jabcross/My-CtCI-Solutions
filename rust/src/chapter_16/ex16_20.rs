...
impl<'a> TrieNode<'a> {
    ...
    fn insert(
        ...
        word: &'a str,
        slice: &'a str,
        letter_to_number: &HashMap<&'a str, &'a str>,
    ) {
        if slice.len() == 0 {
            ...
            if let Some(hashmap) = &mut self.next {
                if hashmap.get_mut(key).is_none() {
    ...
    fn get(&self, numbers: &'a str) -> Option<&Vec<&'a str>> {
        if numbers.len() == 0 {
            Some(&self.terminators)
        } else {
            match &self.next {
                None => None,
                Some(hashmap) => match hashmap.get(&numbers[0..1]) {
                    Some(node) => node.get(&numbers[1..]),
                    None => None,
                },
            }
        }
...
#[test]
fn test() {
    let words: &[&'static str] = &["used", "tree", "car", "road"];
    ...
    assert!(["tree", "used"]
        .iter()
        .all(|x| t9(&trie, "8733").contains(x)));
    assert!(["tree", "used"]
        .iter()
        .all(|x| hasht9.get("8733").contains(x)));
    println!("Ex 16.20 ok!");
}
